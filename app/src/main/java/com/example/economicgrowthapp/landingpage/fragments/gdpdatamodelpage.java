...
public class gdpdatamodelpage extends AppCompatActivity {
    private TextView textViewIndicatorName, textViewPeriodicity, textViewAggregationMethod, indicatortext,
            textViewTopic, textViewLongDefinition,growthrate, textViewStatisticalConcept, accounttext, textViewLimitations;
    private ShimmerFrameLayout shimmerview;
    private LinearLayout maindata;
    private LineChart linechart;
    private CollapsingToolbarLayout toolbar;

    //TODO: do the back end for these,
    // set if the page is on gdp, show the data and graph using put intent a nd stuff.
    // also tell taiwo to help you to do the ui,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        accounttext.setText("Not Avaliable");

// Inside your second activity
        Bundle extras = getIntent().getExtras();
        if (extras != null) {
            String receivedText = extras.getString("gdpamount");
            String receivedText2 = extras.getString("gdpamount2");
            if (receivedText != null) {
                accounttext.setText("N" + receivedText);
                growthrate.setText(receivedText2 + "%");
                if (Double.parseDouble(String.valueOf(receivedText2)) <= 0) {
                    growthrate.setTextColor(Color.RED);
                }
            }
        }
        ...
        toolbar.setTitle("GDP");
        toolbar.setTitle("GROSS DOMESTIC PRODUCT");
        ...
        indicatortext.setText("GROSS DOMESTIC PRODUCT");

        new FetchDataTaskSummary().execute();
        new FetchGdpDataTask().execute();
            ...
            if (jsonObject != null) {
                try {
                    ...
                    if (dataArray.length() > 0) {
                        JSONObject gdpData = dataArray.getJSONObject(3); // Change the index as needed

                        String indicatorName = gdpData.optString("Indicator Name");
                        String periodicity = gdpData.optString("Periodicity");
                        String aggregationMethod = gdpData.optString("Aggregation method");
                        String topic = gdpData.optString("Topic");
                        String longDefinition = gdpData.optString("Long definition");
                        String statisticalConcept = gdpData.optString("Statistical concept and methodology");
                        String limitations = gdpData.optString("Limitations and exceptions");
    ...
    private void setStyledText(TextView textView, String title, String value) {
        ...
        builder.append("\n"); // Separate title from value
        builder.append(value);
        textView.setText(builder);
    }

    private class FetchGdpDataTask extends AsyncTask<Void, Void, JSONObject> {
    ...
    private void createLineGraphForGdp(JSONObject data) {
        try {
            ...
            LineDataSet dataSet = new LineDataSet(entries, "GDP Data");
            dataSet.setColor(Color.BLUE); // Set line color to blue
